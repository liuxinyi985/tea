...
<van-form >
  ...
  />
    ...
    label="确认密码"
    placeholder="密码"
    :rules="[{ validator: (val) => val === registerForm.password, message: '密码不一致' }]"
  />
  <div style="margin: 16px;">
...
export default {
  ...
  methods: {
    ...
    async submitForm(){
      await login({userTel:this.username,userPwd:this.password}).then(res=>{
        console.log(res);
        if(res.code === 200){
    ...
    async register(){
      await addUser({userTel:this.registerForm.username,userPwd:this.registerForm.password}).then(res=>{
        console.log(res);
        if(res.code === 200) {
